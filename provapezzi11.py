...
latitude = 45.85
longitude = 9.40
# Le coordinate per il centro della mappa
lat = 45.89934
lng = 9.44916
...
<html>
  <head>    
	...
	function initMap() {{
		...
		// Passa le variabili Python (mappa_difficolta e localita_json) come JSON a JavaScript
		var mappa_difficolta = {json.dumps(mappa_difficolta)}; 
		...
		locations.forEach(function(loc) {{
		  ...
		  loc.sentieri.forEach(function(sentiero) {{
			...
			if (mappa_difficolta[difficolta]) {{  // ✅ Controllo che esista
			  var colorRGB = mappa_difficolta[difficolta][1];
			  sentieroColor = rgbToHex(colorRGB[0], colorRGB[1], colorRGB[2]);
			  hasFilteredPath = true;
			}} else {{
			  console.warn("Difficoltà non trovata in mappa_difficolta:", difficolta);
			}}
			...
			if (sentiero.coordinata && Array.isArray(sentiero.coordinata)) {{
			  console.log("Coordinata del sentiero:", sentiero.coordinata);

			  var sentieroPath = sentiero.coordinata
				.map(function(coord) {{
				  console.log("Elaborazione coordinata:", coord);
				  var lat = parseFloat(coord[0]);
				  var lng = parseFloat(coord[1]);

				  // Verifica se le coordinate sono numeriche
				  if (isNaN(lat) || isNaN(lng)) {{
					console.warn("Coordinata non valida:", coord);
					return null; // Restituisci null se le coordinate non sono valide
				  }}

				  return {{ lat: lat, lng: lng }}; // Restituisci un oggetto con lat e lng
				}})
				.filter(coord => coord !== null); // Filtra i null
			  ...
			  if (sentieroPath.length > 0) {{
				...
				console.log("Polyline aggiunta per:", sentiero.nome);
			  }}
			}}
		  ...
		  for (var key in mappa_difficolta) {{
			...
			var color = rgbToHex(colorRGB[0], colorRGB[1], colorRGB[2]);
