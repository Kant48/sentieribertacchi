		...
		for nome_sentiero, dati_sentiero in dati_localita["sentieri"].items():
			if scelta_difficolta == "Tutti" or dati_sentiero["difficolta"] == scelta_difficolta:
				...
				for i, passaggio in enumerate(dati_sentiero["percorso"]):
					...
					# Marker con popup e icona
					marker_color = "blue" if i == len(dati_sentiero["percorso"]) - 1 else dati_sentiero["colore"]

					# Cerchiolino con popup
					folium.CircleMarker(
						location=[lat, lng],
						radius=10,  
						color="white",
						fill=True,
						fill_color=dati_sentiero["colore"],
						fill_opacity=0.9,
						tooltip=str(passaggio["sequenza"]),
						popup=folium.Popup(popup_html, max_width=650)  # Popup solo su uno dei due
					).add_to(mappa)
