		...
		for nome_sentiero, dati_sentiero in dati_localita["sentieri"].items():
			if scelta_difficolta == "Tutti" or dati_sentiero["difficolta"] == scelta_difficolta:
				...
				for i, passaggio in enumerate(dati_sentiero["percorso"]):
					...
					# Debug per verificare cosa contiene ogni passaggio
					print(f"Nome Sentiero: {nome_sentiero}")
					print(f"Località: {nome_localita}")
					print(f"Note: {passaggio.get('note', 'Nessuna nota')}")
					print(f"Immagine URL: {passaggio.get('immagine', 'Nessuna immagine')}")
					print(f"Numero Sequenza: {passaggio['sequenza']}")
					...
					"""
                    # Creazione di un cerchiolino più piccolo con il numero della sequenza
					folium.CircleMarker(
						location=[lat, lng],
						radius=10,  # Raggio piccolo per il passaggio
						color="white",  # Bordo nero
						fill=True,
						fill_color=dati_sentiero["colore"],  # Colore del sentiero
						fill_opacity=0.9,
						popup=folium.Popup(popup_html, max_width=650),  # Popup con note e immagine
						tooltip=str(passaggio["sequenza"])  # Mostra il numero al passaggio del mouse
							).add_to(mappa)
					# Aggiunta di una etichetta testuale con il numero della sequenza
					folium.Marker(
						location=[lat, lng],
