...
<html>
  ...
  <body>
	...
	// Dati dal backend (Python)
	var mappa_difficolta = JSON.parse('{{ mappa_difficolta|tojson }}'); // usa il filtro 'tojson' per rendere il JSON sicuro
	var locations = JSON.parse('{{ localita_json|tojson }}');  // usa il filtro 'tojson'
	var scelta_difficolta = "{{ difficolta_filtrata }}";  // difficoltà come stringa
	console.log("Località JSON:", locations);
	console.log("Difficoltà selezionata:", scelta_difficolta);                                                         
	// Aggiunta dei segnaposti
// Aggiunta dei segnaposti
	locations.forEach(function(loc) {{
	  // Per ogni località, iteriamo sui sentieri
	  loc.sentieri.forEach(function(sentiero) {{
		// Determina il colore del segnaposto in base alla difficoltà del sentiero
		var markerColor = "blue";  // Colore di default
		for (var key in mappa_difficolta) {{
		  if (sentiero.difficolta === mappa_difficolta[key][0]) {{
			// Usa il colore definito nella mappa di difficoltà
			markerColor = "rgb(" + mappa_difficolta[key][1].join(",") + ")";
			break;
		  }}
		}}
	  ...
	  var infoWindow = new google.maps.InfoWindow({{
		content: `<h3>${{loc.nome}}</h3>
				  <p>Sentieri disponibili:</p>
				  <ul>` + 
				  loc.sentieri.map(s => 
					`<li>
					  <a href="#" onclick="mostraDettagliSentiero(${{s.id}})">
						${{s.nome}} - Difficoltà: ${{s.difficolta}}
					  </a>
					</li>`
				  ).join('') +
				  `</ul>`
	...
	function mostraDettagliSentiero(id) {{
	  alert("Mostra dettagli per il sentiero con ID: " + id);
	}}
  }}
</script>
