...
for nome_localita in localita:
    for nome_sentiero in localita[nome_localita]["sentieri"]:
        print(f"Sentiero: {nome_sentiero} - Località: {nome_localita}")
        for punto in localita[nome_localita]["sentieri"][nome_sentiero]["percorso"]:
            print(f"  Sequenza: {punto['sequenza']} - Lat: {punto['lat']}, Lng: {punto['lng']}")
...
for nome_localita, dati_localita in localita.items():
    ...
    for i, passaggio in enumerate(dati_sentiero["percorso"]):
        ...
        """
        # Se è l'ultimo punto del sentiero, il marker è blu
        if i == len(dati_sentiero["percorso"]) - 1:
                marker_color = "blue"
        else:
                marker_color = "green"
