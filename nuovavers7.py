...
localita = {}
# Crea il dizionario per i numeri di telefono e le note pratiche
diztelefoni = {}
...
for _, row in df.iterrows():
	...
	print(localita)
   # Dizionario per il numero di telefono e le note pratiche (se esistono)
	diztelefoni[nome_localita] = {
		"numero_telefono": row["Numero Telefono"],
		"note": row.get("NOTELOC", "")  # Aggiungi eventuali note, se esistono
	}
	# Converte il dizionario dei numeri di telefono in formato JSON
...
## Creazione della mappa centrata su coord assegnate
# Centro manuale della mappa (esempio: Lecco, Lombardia)
...
# Converte diztelefoni_jonson in formato JSON e lo "escapa" per usarlo in JavaScript
diztelefoni_json = json.dumps(diztelefoni)  # Converte il dizionario in una stringa JSON
print ("diz jonson")           
print(diztelefoni_json)  # Controlla l'output nella console
...
diztelefoni_json = {
                "Sasso di PREGUDA": {"numero_telefono": "34vvv xxxxxx", "note": ""},
                "RESEGONE": {"numero_telefono": 4875283, "note": ""}
            }

            # Converti il dizionario in una stringa JSON per JavaScript
diztelefoni_js = json.dumps(diztelefoni_json)  # Converte il dizionario in JSON string
...
popup_html = f"""
            ...
            <script>
                // Dizionario con i numeri di telefono passato dal server
                var numeri = {diztelefoni_js}; // Passa il dizionario JSON dal Python
